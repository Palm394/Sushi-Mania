...
public class ShopPane extends VBox implements Updatable {
	...
	public ShopPane() {
		...
		this.getChildren().add(selectedPane());
		this.getChildren().add(buyButtonPane());
	}
	...
	private HBox selectedPane() {
		...
		selectPane.getChildren().addAll(chooseLeftButton(), ingredientImage, chooseRightButton());
		selectPane.setAlignment(Pos.CENTER);
		selectPane.setSpacing(30);
		return selectPane;
	}

	private HBox buyButtonPane() {
		// zone which contain order button
		HBox showPricePane = new HBox();
		Label blank = new Label("");
		blank.setPrefWidth(10);
		showPricePane.getChildren().addAll(normalBuyButton(), blank, expressBuyButton());
		showPricePane.setAlignment(Pos.CENTER);
		return showPricePane;
	}

	private Button normalBuyButton() {
		normalPriceBut = new Button("normal\nprice : " + ingredientList.get(selectedIngredientID).getPrice());
		normalPriceBut.setPrefSize(100, 50);
		normalPriceBut.setTextAlignment(TextAlignment.CENTER);
		...
		return normalPriceBut;
	}

	private Button expressBuyButton() {
		speedPriceBut = new Button("express\nprice : " + (ingredientList.get(selectedIngredientID).getPrice() + 10));
		speedPriceBut.setPrefSize(100, 50);
		speedPriceBut.setTextAlignment(TextAlignment.CENTER);
